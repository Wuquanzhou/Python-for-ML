...
#     f.close()  # 关闭文件
#
#     return np.mat(feature_data), label_tmp, len(set(label_tmp))

def load_competition(file_name):
...
def tan(x):
    ...
    output: tanh值(mat/float):tanh函数的值
    '''
...
def partial_tan(x):
    '''tanh导函数的值
    input:  x(mat/float):自变量，可以是矩阵或者是任意实数
    output: out(mat/float):tanh导函数的值
    '''
    m, n = np.shape(x)
    out = np.mat(np.zeros((m, n)))
    for i in range(m):
        for j in range(n):
            out[i, j] = 1 - tan(x[i, j]) * tan(x[i, j])
    return out
...
def bp_train(feature, label, n_hidden, maxCycle, alpha, n_output):
    ...
    while i <= maxCycle:
        ...
        # 2.2、误差的反向传播
        # 2.2.1、隐含层到输出层之间的残差
        delta_output = -np.multiply((label - output_out), partial_tan(output_in))
        # 2.2.2、输入层到隐含层之间的残差
        delta_hidden = np.multiply((delta_output * w1.T), partial_tan(hidden_input))
...
if __name__ == "__main__":
    ...
    feature, label= load_competition("E:\matlab_work\Competition_train.mat")

    X = feature.reshape(834000, 64)
    y = []
    ...
    w0, w1, b0, b1 = bp_train(np.mat(X), np.mat(y), 20, 1000, 0.1, 2)
    # 3、保存最终的模型
    # print("--------- 3.save model ------------")
    #
